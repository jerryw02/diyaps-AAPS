...
class XdripAidlService(
    ...
    fun getLatestBgDataSync(): BgData? {
        ...
        return try {
            ...
            dataStatistics["sync_remote_errors"] = e.message.orEmpty()
            null
        } catch (e: Exception) {
            aapsLogger.error(LTag.XDRIP,
                "[${TEST_TAG}_GET_SYNC_EXCEPTION] Exception in sync call", e)
            dataStatistics["sync_exceptions"] = e.message.orEmpty()
            null
        }
    }
    ...
    private fun fetchLatestBgData() {
        scope.launch {
            try {
                ...
                if (data != null) {
                    aapsLogger.debug(LTag.XDRIP, "[${TEST_TAG}_LATEST_DATA] Fetched latest data: ${data.glucose}")
                }
            } catch (e: Exception) {
                aapsLogger.error(LTag.XDRIP, "[${TEST_TAG}_FETCH_ERROR] Error fetching latest data", e)
            }
        }
    }

    private fun logDataFlow(step: String, data: BgData, message: String) {
        aapsLogger.debug(LTag.XDRIP, "[${TEST_TAG}_FLOW] $step | $message | Glucose: ${data.glucose}, TS: ${data.timestamp}")
    }
    ...
    private fun handleRemoteException(e: RemoteException) {
        aapsLogger.error(LTag.XDRIP, "[${TEST_TAG}_REMOTE_ERROR] RemoteException", e)
        dataStatistics["remote_exceptions"] = (dataStatistics["remote_exceptions"] as? Int ?: 0) + 1
    }

    private fun handleError(error: String) {
        aapsLogger.error(LTag.XDRIP, "[${TEST_TAG}_ERROR_HANDLER] $error")
