...
// ========== 新增：updateNotification 方法实现 ==========
/**
 * 更新前台服务通知
 * @param data 血糖数据（可选）
 * @param status 连接状态（可选）
 */
private fun updateNotification(data: com.eveningoutpost.dexdrip.BgData? = null, status: String? = null) {
    val ctx = context ?: return
    
    try {
        // 使用 Intent 向前台服务发送更新命令
        val intent = Intent(ctx, XdripForegroundService::class.java).apply {
            action = "UPDATE_NOTIFICATION"
            
            data?.let { bgData ->
                putExtra("glucose", bgData.glucose)
                putExtra("timestamp", bgData.timestamp)
                putExtra("direction", bgData.direction ?: "")
                putExtra("source", bgData.source ?: "")
            }
        ...
        aapsLogger.debug(LTag.BGSOURCE, "[${TEST_TAG}_NOTIFY] Notification update sent")
        
    } catch (e: SecurityException) {
        aapsLogger.error(LTag.BGSOURCE, "[${TEST_TAG}_NOTIFY_SECURITY] Permission error", e)
    } catch (e: Exception) {
        aapsLogger.error(LTag.BGSOURCE, "[${TEST_TAG}_NOTIFY_ERROR] Failed to update notification", e)
    }
}
